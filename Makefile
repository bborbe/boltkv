...
format:
	...
	go run -mod=mod github.com/incu6us/goimports-reviser/v3 -project-name github.com/bborbe/boltkv -format -excludes vendor ./...
...
vulncheck:
	@go run -mod=mod golang.org/x/vuln/cmd/govulncheck -format json $(shell go list -mod=mod ./... | grep -v /vendor/) 2>&1 | \
		jq -e 'select(.finding != null and .finding.osv != "GO-2026-4923")' > /dev/null 2>&1 && \
		{ echo "Unexpected vulnerabilities found"; exit 1; } || \
